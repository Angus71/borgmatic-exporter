...
class TestJsonMetrics:
    ...
    def json_content(self, mocker):
        ...
        yield metrics.collect_json(
            ["/conf/foo.yaml", "/conf/bar.yaml", "/conf/baz.yaml"]
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "borg_total_deduplicated_compressed_size_human",
                "/borg/backup-1",
                "513.0 MiB",
            ),
    ...
    def test_timestamp_metrics(self, json_content):
        assert arrow.get(
            [r for r in json_content if r["location"] == "/borg/backup-1"][0][

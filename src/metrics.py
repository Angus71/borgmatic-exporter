...
def collect_json(borgmatic_configs: list):
    ...
    for r, a in zip(repos, archives):
        current_repo = {
            ...
            "borg_total_compressed_size_human": human_readable_bytes(
                r["cache"]["stats"]["total_csize"]
            ),
            "borg_total_size": r["cache"]["stats"]["total_size"],
            "borg_total_size_human": human_readable_bytes(
                r["cache"]["stats"]["total_size"]
            ),
            "borg_total_deduplicated_compressed_size": r["cache"]["stats"][
                "unique_csize"
            ],
            "borg_total_deduplicated_compressed_size_human": human_readable_bytes(
                r["cache"]["stats"]["unique_csize"]
            ),
            "borg_total_deduplicated_size": r["cache"]["stats"]["unique_size"],
            "borg_total_deduplicated_size_human": human_readable_bytes(
                r["cache"]["stats"]["unique_size"]
            ),
        }
        ...
        if r.get("archives") and len(r["archives"]) > 0:
            ...
            current_repo["borg_last_backup_timestamp"] = (
                arrow.get(latest_archive["end"]).replace(tzinfo="local").timestamp()
            )

            # Last Backup
            current_repo["borg_last_backup_timestamp_human"] = (
                arrow.get(latest_archive["end"]).replace(tzinfo="local").humanize()
            )
            ...
            current_repo["borg_last_backup_deduplicated_compressed_size"] = (
                latest_archive["stats"]["deduplicated_size"]
            )
            current_repo["borg_last_backup_deduplicated_compressed_size_human"] = (
                human_readable_bytes(latest_archive["stats"]["deduplicated_size"])
            )

            # Last Backup Compressed Size
            current_repo["borg_last_backup_compressed_size"] = latest_archive["stats"][
                "compressed_size"
            ]
            current_repo["borg_last_backup_compressed_size_human"] = (
                human_readable_bytes(latest_archive["stats"]["compressed_size"])
            )

            # Last Backup Size
            current_repo["borg_last_backup_size"] = latest_archive["stats"][
                "original_size"
            ]
            current_repo["borg_last_backup_size_human"] = human_readable_bytes(
                latest_archive["stats"]["original_size"]
            )
